...
from src.openlab.persona import build_external_context_text, PARTIES
...
# ── 개별 페르소나 LLM 호출 ────────────────────────────────────
def ask_persona(client, persona, week_info, prev_support, prompt_templates, model, query, provider="openai"):
    fmt = dict(
        profile=persona.get("profile", ""),
        region=persona.get("region", ""),
        week_info=week_info,
...
def simulate_week(client, personas, target_row, prompt_templates, model, provider="openai", query="", st_bar=None):
    """
    [역할] ThreadPoolExecutor를 사용하여 여러 페르소나의 응답을 병렬로 수집하며 Streamlit UI를 업데이트합니다.
    """
    ...
    def run_persona_pipeline(persona):
        res_obj, query_key = ask_persona(
            client, persona, week_info=target_info, prev_support="", 
            prompt_templates=prompt_templates, model=model, provider=provider, query=query
        )
        return persona["persona_id"], query_key, res_obj

    with ThreadPoolExecutor(max_workers=15) as executor:
        futures = [executor.submit(run_persona_pipeline, p) for p in personas]
        
        persona_pbar = tqdm(as_completed(futures), total=total, desc=f"ㄴ 병렬 응답 수집", leave=False, position=1)

        for future in persona_pbar:
            try:
                p_id, q_key, res = future.result()
                if p_id not in aggregated_responses:
                    aggregated_responses[p_id] = {}
                aggregated_responses[p_id][q_key] = res
                
                # 🌟 [핵심] 작업이 하나 완료될 때마다 Streamlit Progress Bar 업데이트
                completed_count += 1
                if st_bar:
                    st_bar.progress(completed_count / total)
